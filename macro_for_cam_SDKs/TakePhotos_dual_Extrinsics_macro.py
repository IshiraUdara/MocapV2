...
# run the script in 'Kinect_OLD' mode
if 'Kinect_OLD' in sys.argv:
    pypreprocessor.defines.append('Kinect_OLD')
...
def acquire_and_display_images_kinect_old(kinect: PyKinectRuntime.PyKinectRuntime, cam_num, flipped=True, floor=False):
    """
    ...
    try:
        ...
        if floor:
            os.makedirs(f'macro_for_cam_SDKs/Kinect_OLD_cam/tracking/cam{cam_num}', exist_ok=True)
        else:
            os.makedirs(f'macro_for_cam_SDKs/Kinect_OLD_cam/captured_images/cam{cam_num}', exist_ok=True)
        ...
        while running.is_set():
            try:
                ...
                if take_photo.is_set():
                    ts = int(time.time())
                    if floor:
                        out_path = f'macro_for_cam_SDKs/Kinect_OLD_cam/tracking/cam{cam_num}/{ts}_ir.png'
                    else:
                        out_path = f'macro_for_cam_SDKs/Kinect_OLD_cam/captured_images/cam{cam_num}/{ts}_ir.png'
...
def run_single_camera_kinect_old(device_id=0, cam_num=0, flipped=True, floor=True):
    """
    ...
    try:
        ...
        result = acquire_and_display_images_kinect_old(kinect, cam_num, flipped, floor)
...
def main_kinect_old(auto=False, floor=False, flipped=True):
    """
    ...
    try:
        ...
        t = threading.Thread(target=run_single_camera_kinect_old, args=(0, 0, flipped, floor))
        t.daemon = True
        t.start()
...
if __name__ == '__main__':
    try:
        success = main_kinect_old(auto=False, floor=False, flipped=True)
        print('Exiting...')
        sys.exit(0 if success else 1)
    except KeyboardInterrupt:
        print("\nProgram interrupted by user")
