...
def get_points(path=None):
    ...
    # Debug: Check the shape before transpose
    print(f"Original shape: {image_points.shape}")
    
    # Only transpose if the array has 3 dimensions
    if len(image_points.shape) == 3:
        image_points = np.transpose(image_points, (1, 0, 2))
    else:
        print(f"Warning: Cannot transpose array with shape {image_points.shape}")
        # Handle 2D case or return as-is
